 ...
 */ //TODO:
static inline uint16_t mavlink_msg_${name_lower}_pack(mavlink_message_t* msg,
...
${{array_fields:    mav_array_memcpy(packet.${name}, ${name}, sizeof(${type})*${array_length});
    ...
    return MAVLINK_MSG_ID_${name}_LEN; //FIXME:
}
 ...
 * @return length of the message in bytes (excluding serial stream start sign)
 */ ////TODO:
static inline uint16_t mavlink_msg_${name_lower}_pack_chan(uint8_t chan, mavlink_message_t* msg,
                                   ${{arg_fields:${array_const}${type} ${array_prefix}${name},}})
{
#if MAVLINK_NEED_BYTE_SWAP || !MAVLINK_ALIGNED_FIELDS
    char buf[MAVLINK_MSG_ID_${name}_LEN];
 ...
 */ //TODO:
static inline uint16_t mavlink_msg_${name_lower}_encode(mavlink_message_t* msg, const mavlink_${name_lower}_t* ${name_lower})
{
    return mavlink_msg_${name_lower}_pack(msg,${{arg_fields: ${name_lower}->${name},}});
}
 ...
 * @param ${name_lower} C-struct to read the message contents from
 */ //TODO:
static inline uint16_t mavlink_msg_${name_lower}_encode_chan(uint8_t chan, mavlink_message_t* msg, const mavlink_${name_lower}_t* ${name_lower})
{
    return mavlink_msg_${name_lower}_pack_chan(chan, msg,${{arg_fields: ${name_lower}->${name},}});
}

// MESSAGE ${name} UNPACKING //FIXME:
...
#else
    memset(${name_lower}, 0, MAVLINK_MSG_ID_${name}_LEN); //FIXME:
...
def copy_fixed_headers(directory, xml):
    ...
    hlist = {
        ...
        "2.0": [ 'protocol.h','mavlink_types.h' ]
        }
